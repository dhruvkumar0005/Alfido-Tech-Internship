...
# ─── AUTO-TRAIN PIPELINE (runs if pkl is missing or incompatible) ────────────────────
def _make_ohe():
    """Return an OneHotEncoder compatible with both old (sparse) and new
    (sparse_output) sklearn API."""
    import sklearn
...
def train_and_save_bundle():
    """Full training pipeline — runs automatically on first deploy."""
    ...
    # Save only if we have write access (won't work on some cloud read-only dirs)
    try:
        os.makedirs(MODELS_DIR, exist_ok=True)
        with open(BUNDLE_PATH, "wb") as f:
            pickle.dump(bundle, f)
    except (OSError, IOError):
        pass  # Read-only filesystem (Streamlit Cloud) — skip saving
...
def _try_load_pickle(path):
    """Safely attempt to load a pickle file. Returns bundle dict or None."""
    try:
        with open(path, "rb") as f:
            return pickle.load(f)
    except Exception:
        # Version mismatch / corrupt / missing module — delete & retrain
        try:
            os.remove(path)
        except OSError:
            pass
...
@st.cache_resource
def load_bundle():
    """Load or train the model bundle. Cached for the session lifetime."""
    # 1. Try absolute path
    if os.path.exists(BUNDLE_PATH):
        result = _try_load_pickle(BUNDLE_PATH)
        if result is not None:
            return result
    # 2. Try relative path
    rel = os.path.join("models", "loan_approval_models.pkl")
    if os.path.exists(rel):
        result = _try_load_pickle(rel)
        if result is not None:
            return result
    # 3. Auto-train from dataset
    return None
