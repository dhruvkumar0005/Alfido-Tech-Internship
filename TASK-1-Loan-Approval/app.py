...
@st.cache_resource
def load_bundle():
    path = os.path.join("models", "loan_approval_models.pkl")
...
if bundle is None:
    st.error("Model bundle not found at `models/loan_approval_models.pkl`. Run `rebuild_bundle.py` or the modeling notebook first.")
    st.stop()
...
with tab2:
    ...
    with img_col1:
        for img_path, caption in [
            ("images/roc_curves_all_models.png", "ROC Curve Comparison"),
            ("images/model_comparison.png",      "All-Metrics Model Comparison")
        ]:
    ...
    with img_col2:
        for img_path, caption in [
            ("images/confusion_matrix.png",  f"{best_model_name} – Confusion Matrix"),
            ("images/feature_importance.png", "Top Feature Contributions")
        ]:
            if os.path.exists(img_path):
                st.image(img_path, caption=caption, use_container_width=True)
...
with tab3:
    ...
    img_pairs = [
        ("images/target_distribution.png",   "Class Imbalance (Y=Approved, N=Rejected)"),
        ("images/missing_values_heatmap.png", "Missing Values Heatmap"),
        ("images/credit_history_impact.png",  "Credit History → Approval Rate"),
        ("images/property_area_impact.png",   "Property Area → Approval Rate"),
        ("images/numerical_distributions.png","Numerical Feature Distributions"),
        ("images/smote_class_balance.png",    "SMOTE Balancing Effect"),
        ("images/correlation_heatmap.png",    "Correlation Between Numerical Features"),
    ]
